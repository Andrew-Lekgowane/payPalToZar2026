...
const ALL_STATUSES = ["all", "pending", "verifying", "processing", "completed", "failed", "refunded"];
...
export default function AdminPage() {
  ...
  // Stats
  const totalZARPaid = transactions.filter(t => t.status === "completed").reduce((a, t) => a + t.amountZAR, 0);
  const pendingCount = transactions.filter(t => ["pending", "verifying"].includes(t.status)).length;
  const completedCount = transactions.filter(t => t.status === "completed").length;

  const filteredTx = statusFilter === "all" ? transactions : transactions.filter(t => t.status === statusFilter);
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-500 mt-1">Manage transactions, process payouts, and manage users</p>
            </div>
            <button onClick={fetchData} className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800" title="Refresh">
            ...
            <Card>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-emerald-600">R{totalZARPaid.toLocaleString("en-ZA", { minimumFractionDigits: 2 })}</p>
                </div>
              </div>
            </Card>
          </div>
            ...
            {(["transactions", "users"] as const).map((t) => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${
                  tab === t ? "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300" : "text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800"
                }`}>
                {t}
                {t === "transactions" && pendingCount > 0 && (
                  <span className="ml-2 px-1.5 py-0.5 rounded-full bg-amber-500 text-white text-xs font-bold">{pendingCount}</span>
                )}
              </button>
            ))}
          </div>
                ...
                {ALL_STATUSES.map((s) => (
                  <button key={s} onClick={() => setStatusFilter(s)}
                    className={`px-3 py-1 rounded-full text-xs font-medium capitalize transition-colors ${
                      statusFilter === s ? "bg-violet-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-violet-100 dark:hover:bg-violet-900/30"
                    }`}>
                    {s}
                    {s !== "all" && (
                      <span className="ml-1 opacity-70">({transactions.filter(t => t.status === s).length})</span>
                    )}
                  </button>
                ))}
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800">
                      ...
                      <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase">USD</th>
                      <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase">ZAR Payout</th>
                      <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase">Proof</th>
                  ...
                  <tbody>
                    {filteredTx.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="py-10 text-center text-gray-400 text-sm">No transactions found</td>
                      </tr>
                    ) : filteredTx.map((tx) => {
                      ...
                      return (
                        <tr key={tx._id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                          ...
                          <td className="py-3 px-3 text-sm">
                            <p className="font-medium text-gray-900 dark:text-white">{tx.userId?.name || "Unknown"}</p>
                            <p className="text-xs text-gray-500">{tx.userId?.email}</p>
                          </td>
                          <td className="py-3 px-3 text-sm font-medium text-gray-900 dark:text-white">${tx.amountUSD.toFixed(2)}</td>
                          <td className="py-3 px-3 text-sm font-bold text-emerald-600">R{tx.amountZAR.toFixed(2)}</td>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800">
                      ...
                      <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase">Bank</th>
                  ...
                  <tbody>
                    {users.length === 0 ? (
                      <tr><td colSpan={7} className="py-10 text-center text-gray-400 text-sm">No users found</td></tr>
                    ) : users.map((u) => (
                      <tr key={u._id} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                        <td className="py-3 px-3 text-sm font-medium text-gray-900 dark:text-white">{u.name}</td>
                        <td className="py-3 px-3 text-sm text-gray-600 dark:text-gray-300">{u.email}</td>
                        <td className="py-3 px-3 text-sm text-gray-600 dark:text-gray-300">{u.phone || "—"}</td>
                        <td className="py-3 px-3 text-sm text-gray-600 dark:text-gray-300">{u.bankName || "—"}</td>
                        <td className="py-3 px-3"><Badge variant={u.role === "admin" ? "info" : "default"}>{u.role}</Badge></td>
                        <td className="py-3 px-3 text-sm text-gray-500">{new Date(u.createdAt).toLocaleDateString("en-ZA")}</td>
                        <td className="py-3 px-3">
                          <button onClick={() => setSelectedUser(u)} className="p-1.5 rounded-lg text-violet-500 hover:bg-violet-50 dark:hover:bg-violet-900/30" title="Manage user">
                            <Eye className="w-4 h-4" />
                          </button>
                        </td>
                      </tr>
      ...
      <Modal isOpen={!!selectedTx} onClose={() => { setSelectedTx(null); setAdminNote(""); setShowScreenshot(false); setPendingAction(null); }} title="Process Transaction" size="lg">
        {selectedTx && (
          <div className="space-y-5">
            ...
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800 grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p className="font-semibold text-gray-900 dark:text-white">{selectedTx.userId?.name || "Unknown"}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-0.5">Phone</p>
                <p className="font-medium text-gray-900 dark:text-white">{selectedTx.userId?.phone || "—"}</p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-gray-500 mb-0.5">Email</p>
                <p className="font-medium text-gray-900 dark:text-white">{selectedTx.userId?.email}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 text-sm">
              <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-center">
                ...
                <p className="font-bold text-blue-700 dark:text-blue-300 text-lg">${selectedTx.amountUSD.toFixed(2)}</p>
              </div>
              <div className="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-center">
                <p className="text-xs text-red-500 mb-1">Service Fee</p>
                <p className="font-bold text-red-600 text-lg">-${selectedTx.serviceFee.toFixed(2)}</p>
              </div>
              <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 text-center">
                <p className="text-xs text-emerald-600 mb-1">You Must Pay Out</p>
                <p className="font-bold text-emerald-700 dark:text-emerald-300 text-lg">R{selectedTx.amountZAR.toFixed(2)}</p>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border-2 border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/10">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-bold text-emerald-700 dark:text-emerald-300">Payout Bank Details — EFT to these details</p>
              </div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Bank</p>
                  <div className="flex items-center">
                    <p className="font-semibold text-gray-900 dark:text-white">{selectedTx.bankName || "—"}</p>
                  </div>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Account Holder</p>
                  <div className="flex items-center">
                    <p className="font-semibold text-gray-900 dark:text-white">{selectedTx.accountHolder || "—"}</p>
                    {selectedTx.accountHolder && <CopyButton value={selectedTx.accountHolder} label="Account holder" />}
                  </div>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Account Number</p>
                  <div className="flex items-center">
                    <p className="font-bold text-gray-900 dark:text-white font-mono text-base">{selectedTx.accountNumber || "—"}</p>
                    {selectedTx.accountNumber && <CopyButton value={selectedTx.accountNumber} label="Account number" />}
                  </div>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Branch Code</p>
                  <div className="flex items-center">
                    <p className="font-semibold text-gray-900 dark:text-white font-mono">{selectedTx.branchCode || "—"}</p>
                    {selectedTx.branchCode && <CopyButton value={selectedTx.branchCode} label="Branch code" />}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800 text-sm">
              <p className="text-xs text-gray-500 mb-1">PayPal Transaction ID</p>
              <div className="flex items-center">
                <p className="font-mono text-gray-900 dark:text-white break-all">
                  {selectedTx.paypalTransactionId || "Not provided"}
                </p>
                {selectedTx.paypalTransactionId && <CopyButton value={selectedTx.paypalTransactionId} label="Transaction ID" />}
              </div>
            </div>
            ...
            {selectedTx.proofScreenshot ? (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Payment Screenshot</p>
                  <button
              ...
              {pendingAction ? (
                <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 space-y-3">
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-200">
                    Change status to <span className="capitalize">&quot;{pendingAction.value}&quot;</span>?
                  </p>
                  ...
                  <div className="flex gap-2">
                    <Button
                      onClick={() => { setPendingAction(null); updateStatus(selectedTx, pendingAction.value); }}
                    ...
                    <Button variant="outline" onClick={() => setPendingAction(null)} disabled={updatingStatus !== null}>
                      Cancel
                    </Button>
                  </div>
                </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      variant="outline"
                      onClick={() => setPendingAction({ type: "status", value: "verifying" })}
                    ...
                    <Button
                      variant="outline"
                      onClick={() => setPendingAction({ type: "status", value: "processing" })}
                    ...
                    <Button
                      variant="secondary"
                      onClick={() => setPendingAction({ type: "status", value: "completed" })}
                    ...
                    <Button
                      variant="danger"
                      onClick={() => setPendingAction({ type: "status", value: "failed" })}
                    ...
                    <Button
                      variant="outline"
                      onClick={() => setPendingAction({ type: "status", value: "refunded" })}
      ...
      <Modal isOpen={!!selectedUser} onClose={() => { setSelectedUser(null); setPendingUserAction(null); }} title="Manage User" size="lg">
        {selectedUser && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800">
                <p className="text-xs text-gray-500 mb-0.5">Bank</p>
                <p className="font-semibold text-gray-900 dark:text-white">{selectedUser.bankName || "—"}</p>
              </div>
              <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800">
                <p className="text-xs text-gray-500 mb-0.5">Account Number</p>
                <div className="flex items-center">
                  <p className="font-semibold text-gray-900 dark:text-white font-mono">{selectedUser.accountNumber || "—"}</p>
                  {selectedUser.accountNumber && <CopyButton value={selectedUser.accountNumber} label="Account number" />}
                </div>
              </div>
              <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800">
              ...
              <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800">
                ...
                <p className="font-semibold text-gray-900 dark:text-white">{new Date(selectedUser.createdAt).toLocaleDateString("en-ZA")}</p>
              </div>
            </div>
              ...
              {pendingUserAction ? (
                ...
                <>
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Change Role</p>
                    <div className="flex gap-2">
                      <Button
                        onClick={() => setPendingUserAction({ type: "role", value: "admin" })}
                      ...
                      <Button
                        variant="outline"
                        onClick={() => setPendingUserAction({ type: "role", value: "user" })}
